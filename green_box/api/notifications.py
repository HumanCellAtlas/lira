...
def post(body):
    ...
    if len(id_matches) == 0:
        return response_with_server_header(dict(
            error='Not Found: No wdl config found with subscription id {}'.format(
                subscription_id)), 404)
    elif len(id_matches) > 1:
        return response_with_server_header(dict(
            error='Internal Server Error: More than one wdl config found with subscription id {}'
                  ''.format(subscription_id)), 500)
    ...
    subprocess.check_output(['gsutil', 'cp', wdl['default_inputs_link'], '.'])
    subprocess.check_output(['gsutil', 'cp', wdl['wdl_deps_link'], '.'])

    wdl_file = os.path.split(wdl['wdl_link'])[-1]
    wdl_deps_file = os.path.split(wdl['wdl_deps_link'])[-1]
    wdl_default_inputs_file = os.path.split(wdl['default_inputs_link'])[-1]
    cromwell_response = start_workflow(
        wdl_file, wdl_deps_file, 'cromwell_inputs.json', wdl_default_inputs_file, green_config)
...
def compose_inputs(workflow_name, uuid, version, provenance_script):
    return {workflow_name + '.bundle_uuid': uuid,
            ...
            workflow_name + '.provenance_script': provenance_script}
...
def start_workflow(wdl_file, zip_file, inputs_file, inputs_file2, green_config):
    with open(wdl_file, 'rb') as wdl, open(zip_file, 'rb') as deps, open(inputs_file, 'rb') as inputs, open(inputs_file2, 'rb') as inputs2:
        ...
        response = requests.post(
            ...
            auth=HTTPBasicAuth(green_config.cromwell_user, green_config.cromwell_password)
        )
        return response
