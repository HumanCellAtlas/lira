...
if [ -z $lira_secret ]; then
  echo "lira_secret is required"
  error=1
fi
...
if [ "$error" == "1" ]; then
  echo -e "Usage: bash subscription-create.sh dss_url green_url key_file lira_secret query_json [hmac_key_id] [additional_metadata]\n"
  exit 1
fi
...
python3 subscription-create.py \
